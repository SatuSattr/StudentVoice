  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setCategory(post.tag_kategori);
        setLocation(post.tag_location ?? '');
        if (post.photo_video) {
          setExistingMediaUrl(post.photo_video);
        }
      } catch (err) {
        Alert.alert('Error', 'Gagal memuat data postingan.');
        router.back();
      } finally {
        setFetching(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data?.errors) {
        ...
        Alert.alert('Error', data?.message ?? 'Gagal memperbarui postingan');
      }
    } finally {
      setLoading(false);
    }
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.toolbarWrap}>
            <TouchableOpacity style={styles.toolbarBtn} onPress={pickMedia}>
              ...
              <Text style={styles.toolbarBtnText}>{existingMediaUrl ? 'Ganti Foto/Video' : 'Tambah Foto/Video'}</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
